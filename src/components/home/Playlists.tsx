...
export default function Playlists({
  ...
  const [playlistPage, setPlaylistPage] =
    useState<Page<SimplifiedPlaylist> | null>(null);
  ...
  return playlists?.map((playlist) => {
    ...
    return (
      <div
        ...
        key={playlist.id}
        className="my-2 mx-2 p-4 transition-colors duration-200 ease-in-out bg-transparent rounded-lg hover:bg-[#ffffff0a] cursor-pointer"
        onClick={() =>
          (window.location.href = `/playlist-viewer/${playlist.id}`)
        }
      >
        {image ? (
        ...
        <p className="text-white overflow-hidden whitespace-nowrap text-ellipsis w-36">
          {playlist.name}
        </p>
