...
export function NewComposition() {
  function click() {
    window.location.href = "/composition-lister"
  }
...
export default function Compositions({compositionCount = 4} : {compositionCount?: number}) {
  ...
  {compositions.map(composition => (
      <div key={composition.id} className="my-4 mx-2 p-4 transition-colors duration-200 ease-in-out bg-transparent rounded-lg hover:bg-[#ffffff0a] cursor-pointer" onClick={() => window.location.href = `/composition-test/${composition.id}`}>
