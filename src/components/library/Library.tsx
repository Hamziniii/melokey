...
function Pill({
  ...
  return active ? (
    <div
      className="text-sm font-thin text-black bg-neutral-100 py-1 px-3 rounded-2xl cursor-pointer select-none"
      onClick={onClick}
    >
      {text}
    </div>
    ...
    <div
      className="text-sm font-thin text-gray-400 transition-colors bg-neutral-800 py-1 px-3 rounded-2xl duration-150 ease-in-out hover:text-white cursor-pointer select-none"
      onClick={onClick}
    >
      {text}
    </div>
...
function PlaylistTile({ playlist }: { playlist: PartialPlaylist }) {
  ...
  return (
    <div
      className="flex flex-row w-full gap-2 h-12 cursor-pointer"
      onClick={() => {
        if (playlist) window.location.href = "/playlist-viewer/" + playlist.id;
      }}
    >
      {image ? (
      ...
      <div className="flex flex-col w-full overflow-hidden">
        <h2 className="text-base w-full overflow-hidden whitespace-nowrap text-ellipsis text-gray-400 transition-colors duration-150 ease-in-out hover:text-white">
          {playlist?.name ?? "No name"}
        </h2>
        <h3 className="text-sm font-thin text-gray-300">
...
function TagTile({ tag }: { tag: TagWithTracks }) {
  return (
    <div
      className="flex flex-row w-full gap-2 h-12 cursor-pointer"
      onClick={() => {
        if (tag) window.location.href = "/tag-viewer/" + tag.id;
      }}
    >
      <div
        className="w-12 h-12 rounded-md flex-shrink-0 text-gray-500 bg-gradient-to-b from-slate-600"
...
export default function Library({
  ...
  const [playlists, setPlaylists] = useState<PartialPlaylist[]>(
    createPlaylistPlaceholders(playlistCount || 4),
  );
  const [tags, setTags] = useState<TagWithTracks[]>([]);
  const [compositions, setCompositions] = useState<Composition[]>([]);
  const [active, setActive] = useState<"Playlists" | "Tags" | "Compositions">(
    "Playlists",
  );
  const pills: ["Playlists", "Tags", "Compositions"] = [
    "Playlists",
    "Tags",
    "Compositions",
  ];
  ...
  useEffect(() => {
    localStorage.getItem("sideNavPlaylists") &&
      setPlaylists(JSON.parse(localStorage.getItem("sideNavPlaylists")!));
